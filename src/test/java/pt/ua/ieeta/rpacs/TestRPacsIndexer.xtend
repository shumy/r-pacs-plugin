...
class TestRPacsIndexer {
	...
	def void indexAndQueryDicomFile() {
		...
		val sResults = query.queryText('name:Anonymized and  sex:  M and   studies.accessionNumber: R051961172  ')
		Assert.assertEquals(4, sResults.size)
		
		val result0 = sResults.get(0).extraData
		val result1 = sResults.get(1).extraData
		val result2 = sResults.get(2).extraData
		val result3 = sResults.get(3).extraData
